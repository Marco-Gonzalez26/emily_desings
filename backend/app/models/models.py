from sqlalchemy import (
    Boolean, Column, String, Integer, Numeric, Text, 
    DateTime, ForeignKey, CheckConstraint, UniqueConstraint, text
)
...
class Usuario(Base):
    ...
    __table_args__ = (
        CheckConstraint("rol IN ('cliente', 'administrador')", name="usuarios_rol_check"),
    )
    ...
    perfil_morfologico = relationship("PerfilMorfologico", back_populates="usuario", uselist=False, cascade="all, delete-orphan")
    preferencias_estilo = relationship("PreferenciaEstilo", back_populates="usuario", cascade="all, delete-orphan")
    analisis_morfologico = relationship("AnalisisMorfologico", back_populates="usuario", cascade="all, delete-orphan")
    ...
    productos_creados = relationship("Producto", foreign_keys="Producto.administrador_id", back_populates="administrador")
    historial_productos = relationship("HistorialProducto", foreign_keys="HistorialProducto.administrador_id", back_populates="administrador")
    historial_reportes = relationship("HistorialReporte", back_populates="administrador")
    recomendaciones_ia = relationship("RecomendacionIA", back_populates="usuario", cascade="all, delete-orphan")
...
class PerfilMorfologico(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    usuario_id = Column(UUID(as_uuid=True), ForeignKey("usuarios.id", ondelete="CASCADE"), unique=True)
    altura = Column(Numeric(5, 2))
    peso = Column(Numeric(5, 2))
    tipo_cuerpo = Column(String(50))
    fecha_ultima_actualizacion = Column(DateTime, server_default=text("CURRENT_TIMESTAMP"))
    created_at = Column(DateTime, server_default=text("CURRENT_TIMESTAMP"))
    updated_at = Column(DateTime, server_default=text("CURRENT_TIMESTAMP"))
...
class PreferenciaEstilo(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("usuario_id", "estilo", name="preferencias_estilo_usuario_id_estilo_key"),
    )
...
class AnalisisMorfologico(Base):
    """Historial de análisis morfológicos realizados por IA"""
    __tablename__ = "analisis_morfologico"
    
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    usuario_id = Column(UUID(as_uuid=True), ForeignKey("usuarios.id", ondelete="CASCADE"))
    url_imagen_temporal = Column(String(500))
    tipo_cuerpo_detectado = Column(String(50))
    fecha_analisis = Column(DateTime, server_default=text("CURRENT_TIMESTAMP"))
    imagen_eliminada = Column(Boolean, default=False)
    fecha_eliminacion_imagen = Column(DateTime)
    
    
    usuario = relationship("Usuario", back_populates="analisis_morfologico")
...
class Producto(Base):
    ...
    etiquetas_morfologicas = relationship("ProductoEtiquetaMorfologica", back_populates="producto", cascade="all, delete-orphan")
    ...
    historial = relationship("HistorialProducto", back_populates="producto", cascade="all, delete-orphan")
    recomendaciones_ia = relationship("RecomendacionIA", back_populates="producto", cascade="all, delete-orphan")
...
class ProductoEtiquetaMorfologica(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("producto_id", "tipo_cuerpo", name="producto_etiquetas_morfologicas_producto_id_tipo_cuerpo_key"),
    )
...
class Inventario(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("stock_reservado >= 0", name="inventario_stock_reservado_check"),
        UniqueConstraint("producto_id", "talla_id", "color_id", name="inventario_producto_id_talla_id_color_id_key"),
    )
...
class CarritoItem(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    carrito_id = Column(UUID(as_uuid=True), ForeignKey("carritos.id", ondelete="CASCADE"))
...
class Orden(Base):
    ...
    comprobante = relationship("Comprobante", back_populates="orden", uselist=False, cascade="all, delete-orphan")
...
class Comprobante(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("uuid_generate_v4()"))
    orden_id = Column(UUID(as_uuid=True), ForeignKey("ordenes.id", ondelete="CASCADE"), unique=True)
...
class HistorialProducto(Base):
    ...
    administrador = relationship("Usuario", foreign_keys=[administrador_id], back_populates="historial_productos")
